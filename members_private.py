...
def display_members_private():
    ...
    if 'notify_user' not in st.session_state:
        st.session_state['notify_user'] = False
    ...
    with st.session_state['tabs'][0]:
        st.markdown(
            ...
            "<h3 style='margin-top: 0; color: black; font-size: 16px;'>Members - Private Individuals</h3>",
            unsafe_allow_html=True
        )

        st.markdown("<div style='border: 1px solid #E0E0E0; border-radius: 4px; padding: 8px; margin-bottom: 8px;'>", unsafe_allow_html=True)
        st.markdown("<h4 style='font-size: 14px; font-weight: 500; margin-top: 0; color: black;'>Activity Summary</h4>", unsafe_allow_html=True)
    ...
    with st.session_state['tabs'][1]:
        st.markdown(
            "<div style='border: 1px solid #E0E0E0; border-radius: 4px; padding: 8px; margin-bottom: 8px;'>"
            "<h3 style='margin-top: 0; color: black; font-size: 16px;'>Manage Members Activities</h3>",
            unsafe_allow_html=True
        )

        st.markdown("<h4 style='font-size: 14px; font-weight: 500; margin-top: 0; color: black;'>Active Activities</h4>", unsafe_allow_html=True)
        columns = [
        ...
        for activity in activities:
            ...
            if st.session_state.get(edit_key):
                with st.form(key=f"edit_activity_form_{activity['id']}"):
                    st.markdown(f"<h3 style='color: black; font-size: 16px;'>Edit Activity: {activity['activity']}</h3>", unsafe_allow_html=True)
            ...
            if st.session_state.get(delete_key):
                with st.form(key=f"delete_activity_form_{activity['id']}"):
                    st.markdown(f"<h3 style='color: black; font-size: 16px;'>Move Activity to Non-Active: {activity['activity']}</h3>", unsafe_allow_html=True)
                    st.markdown("<p style='color: black; font-size: 14px;'>Are you sure you want to move this activity to Non-Active Projects?</p>", unsafe_allow_html=True)
        ...
        st.markdown("<h4 style='font-size: 14px; font-weight: 500; margin-top: 8px; color: black;'>Non-Active Projects</h4>", unsafe_allow_html=True)
        columns = [
            {"name": "Object ID", "field": "id"},
        ...
        for activity in non_active_activities:
            ...
            if st.session_state.get(restore_key):
                with st.form(key=f"restore_activity_form_{activity['id']}"):
                    st.markdown(f"<h3 style='color: black; font-size: 16px;'>Restore Activity: {activity['activity']}</h3>", unsafe_allow_html=True)
                    st.markdown("<p style='color: black; font-size: 14px;'>Are you sure you want to restore this activity to Active Projects?</p>", unsafe_allow_html=True)
            ...
            if st.session_state.get(permanent_delete_key):
                with st.form(key=f"permanent_delete_activity_form_{activity['id']}"):
                    st.markdown(f"<h3 style='color: black; font-size: 16px;'>Permanently Delete Activity: {activity['activity']}</h3>", unsafe_allow_html=True)
                    st.markdown("<p style='color: black; font-size: 14px;'>Are you sure you want to permanently delete this activity? This action cannot be undone.</p>", unsafe_allow_html=True)
    ...
    with st.session_state['tabs'][2]:
        st.markdown(
            "<div style='border: 1px solid #E0E0E0; border-radius: 4px; padding: 8px; margin-bottom: 8px;'>"
            "<h3 style='margin-top: 0; color: black; font-size: 16px;'>Checklist Management</h3>",
            unsafe_allow_html=True
        )

        st.markdown("<h4 style='font-size: 14px; font-weight: 500; margin-top: 0; color: black;'>Select Object to Manage</h4>", unsafe_allow_html=True)
        ...
        if selected_object_id:
            st.markdown(f"<h4 style='font-size: 14px; font-weight: 500; margin-top: 8px; color: black;'>Checklist for Object ID: {selected_object_id}</h4>", unsafe_allow_html=True)
            render_checklist(checklist_items, selected_object_id, log_action)

        st.markdown("<h4 style='font-size: 14px; font-weight: 500; margin-top: 8px; color: black;'>Completed Projects</h4>", unsafe_allow_html=True)
        ...
        for activity in completed_activities:
            ...
            if st.session_state.get(edit_completed_key):
                with st.form(key=f"edit_completed_form_{activity['id']}"):
                    st.markdown(f"<h3 style='color: black; font-size: 16px;'>Edit Completed Activity: {activity['activity']}</h3>", unsafe_allow_html=True)
                    new_activity_type = st.text_input("Activity Type", value=activity['activity'])
                    new_activity_date = st.text_input("Date (YYYY-MM-DD)", value=activity['date'])
                    new_activity_amount = st.text_input("Amount (optional)", value=activity['amount'] if activity['amount'] else "")
                    edit_submit = st.form_submit_button("Save Changes")
            ...
            if st.session_state.get(delete_completed_key):
                with st.form(key=f"delete_completed_form_{activity['id']}"):
                    st.markdown(f"<h3 style='color: black; font-size: 16px;'>Delete Completed Activity: {activity['activity']}</h3>", unsafe_allow_html=True)
                    st.markdown("<p style='color: black; font-size: 14px;'>Are you sure you want to permanently delete this activity? This action cannot be undone.</p>", unsafe_allow_html=True)
                    confirm_delete = st.text_input("Type the activity name to confirm", placeholder=activity['activity'])
