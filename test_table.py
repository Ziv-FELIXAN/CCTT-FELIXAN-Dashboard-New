...
def display_test_table():
    ...
    # Display the DataFrame with a reduced width
    with st.container():
        col1, col2 = st.columns([3, 1])  # Adjust the ratio to leave space on the right
        with col1:
        ...
        with col2:
            for idx, item in enumerate(st.session_state['test_data']):
                # Add some vertical spacing to align buttons with rows
